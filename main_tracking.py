...
def show_body(frame):
		...
		if results.pose_landmarks is not None:
			...
			right_hand = (int(keypoint_dict['RIGHT_WRIST'][0] * frame.shape[1]), int(keypoint_dict['RIGHT_WRIST'][1] * frame.shape[0]))
			...
			for part, coords in {'right_shoulder': right_shoulder, 'left_shoulder': left_shoulder, 'head': head, 'right_foot': right_foot, 'left_foot': left_foot, 'right_hand': right_hand, 'left_hand': left_hand, 'right_knee': right_knee, 'left_knee': left_knee}.items():
				x, y = coords
...
part_labels = {'right_shoulder': 'RS', 'left_shoulder': 'LS', 'head': 'H', 'right_foot': 'RF', 'left_foot': 'LF', 'right_hand': 'RH', 'left_hand': 'LH', 'right_knee': 'RK', 'left_knee': 'LK'}
...
# to the webcam
if not args.get("video", False): vs = VideoStream(src=0).start()
...
while True:
	...
	if SHOW_BODY:
		...
		if results.pose_landmarks is not None:
			...
			# Add boxes with part labels
			overlapping_boxes = []
			part_coords = {'right_shoulder': right_shoulder, 'left_shoulder': left_shoulder, 'head': head, 'right_foot': right_foot, 'left_foot': left_foot, 'right_hand': right_hand, 'left_hand': left_hand, 'right_knee': right_knee, 'left_knee': left_knee}
			for part1, coords1 in part_coords.items():
				x1, y1 = coords1
				x10, y10 = max(x1 - box_size // 2, 0), max(y1 - box_size // 2, 0)
				x11, y11 = min(x1 + box_size // 2, frame.shape[1]), min(y1 + box_size // 2, frame.shape[0])
				cv2.rectangle(frame, (x10, y10), (x11, y11), box_color, -1)
				label = part_labels[part1]
				label_size, baseline = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 1.5, 4)
				label_x, label_y = x1 - label_size[0] // 2, y1 + box_size // 2 + label_size[1] // 2
				cv2.putText(frame, label, (label_x, label_y), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 0), 4, cv2.LINE_AA)
